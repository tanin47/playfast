<script lang="ts">
import Button from '../common/_button.svelte'
import {FetchError, post} from "../common/form";
import ErrorPanel from '../common/form/_error_panel.svelte';

export let userId: string
export let secretToken: string

let isLoading = false
let errors: string[] = []

async function submit(): Promise<void> {
  isLoading = true
  try {
    const _json = await post('/verify-email', {userId, secretToken})

    window.location.href = '/'
  } catch (e) {
    isLoading = false
    errors = (e as FetchError).messages
  }
}
</script>

<div class="hero bg-base-200 min-h-screen">
  <div class="hero-content flex-col justify-center items-center">
    <div class="card bg-base-100 min-w-[400px] w-full max-w-sm shrink-0 shadow-2xl">
      <div class="card-body">
        <div class="flex flex-col gap-4">
          <span>Thank you for signing up with Fingertip AI.</span>
          <span>Please click the below button to verify your email:</span>
          <Button {isLoading} onClick={submit} dataTestId="submit-button">Click to verify your email</Button>
          <ErrorPanel {errors}/>
        </div>
      </div>
    </div>
  </div>
</div>


<style lang="scss">
</style>
